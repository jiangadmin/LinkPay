...
public class HttpUtil {
    ...
    public static String doGet(Map<String, String> params) {
        ...
        for (Map.Entry<String, String> entry : params.entrySet()) {
            if ("null".equals(entry.getValue()) || entry.getValue() == null) {
                continue;
            }
        ...
        try {
            if (Const.HTTPDES) {
                LogUtil.e(TAG, "加密前 doGet : " + Const.URL + "?" + paramsStr);
                url = new URL(Const.URL + "?" + DESedeTool.encryptDES(paramsStr, Const.HTTPKEY).replaceAll(" ", ""));

            } else {
                url = new URL(Const.URL + "?" + paramsStr);
            }
            LogUtil.e(TAG, "加密后 doGet : " + url);
            ...
            if (conn.getResponseCode() == 200) {
                ...
                if (Const.HTTPDES) {
                    ...
                    String re = DESedeTool.decryptDES(baos.toString(), Const.HTTPKEY);
                    LogUtil.e(TAG, "HTTP返回: " + re);
                    return re;
                }
    ...
    public static String doPostforDES(String url, Map<String, String> param) {
        ...
        try {
            ...
            if (Const.HTTPDES) {
                String DESStr = URLEncoder.encode(DESedeTool.encryptDES(paramStr.toString(), Const.HTTPKEY).replaceAll(" ", ""));
                out.print(DESStr);
                LogUtil.e(TAG, "DES后发送 " + url + "?" + DESStr);
            } else {
                out.print(paramStr);
            }
            // flush输出流的缓冲
        ...
        if (result != null) {
            if (Const.HTTPDES) {
                re = DESedeTool.decryptDES(URLDecoder.decode(result), Const.HTTPKEY) == null ? null : DESedeTool.decryptDES(URLDecoder.decode(result), Const.HTTPKEY);
            } else {
                re = result == null ? null : result;
            }
        } else {
            re = result;
        }
